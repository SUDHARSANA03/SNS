...
export const InteractiveBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // ─── 3D HOLOGRAPHIC PARTICLES (Violet Theme) ───
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const colors = [
        'rgba(199, 125, 255, ',  // Electric Violet (#C77DFF)
        'rgba(168, 85, 247, ',   // Deep Purple (#A855F7)
        'rgba(232, 121, 249, '   // Orchid Violet (#E879F9)
      ];
    ...
    const render = () => {
      ...
      // Match obsidian background
      ctx.fillStyle = '#0A0810';
      ctx.fillRect(0, 0, W, H);

      // ─── 1. AMBIENT GLOWS (Violet Aura) ───
      ...
      violetGlow.addColorStop(0, 'rgba(199, 125, 255, 0.06)');
      violetGlow.addColorStop(0.5, 'rgba(168, 85, 247, 0.02)');
      ...
      purpleGlow.addColorStop(0, 'rgba(147, 51, 234, 0.03)');
      purpleGlow.addColorStop(1, 'transparent');
      ctx.fillStyle = purpleGlow;
      ctx.fillRect(0, 0, W, H);

      // ─── 2. PERSPECTIVE GRID (Violet Trace) ───
      ctx.strokeStyle = 'rgba(199, 125, 255, 0.025)';
      ctx.lineWidth = 1;
      ...
      for (let z = stepZ; z < horizonZ; z += stepZ) {
        ...
        const alpha = Math.max(0, 1 - animatedZ / horizonZ) * 0.45;
        ctx.strokeStyle = `rgba(199, 125, 255, ${alpha * 0.045})`;
        ctx.beginPath();
      ...
      radarWaves.forEach(wave => {
        ...
        ctx.strokeStyle = `rgba(199, 125, 255, ${wave.alpha})`;
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.arc(radarCenterX, radarCenterY, wave.radius, 0, Math.PI * 2);
        ctx.stroke();

        ctx.strokeStyle = `rgba(232, 121, 249, ${wave.alpha * 0.3})`;
        ctx.lineWidth = 0.5;
        ctx.beginPath();
      ...
      nodes.forEach((node, i) => {
        node.connections.forEach(connIdx => {
          if (connIdx > i) {
            ...
            ctx.strokeStyle = 'rgba(199, 125, 255, 0.02)';
            ctx.stroke();
          }
        });
        ...
        if (node.pulseActive) {
          ...
          if (node.pulseTarget !== -1) {
            ...
            if (node.pulseProgress >= 1) {
              ...
              const colorSelector = i % 3 === 0 ? 'rgba(232, 121, 249, 0.75)' : 'rgba(199, 125, 255, 0.75)';
              ...
              pGlow.addColorStop(0, i % 3 === 0 ? 'rgba(232, 121, 249, 0.2)' : 'rgba(199, 125, 255, 0.2)');
        ...
        ctx.fillStyle = i % 4 === 0 ? `rgba(232, 121, 249, ${alphaPulse})` : `rgba(199, 125, 255, ${alphaPulse})`;
        ctx.beginPath();
        ctx.arc(node.x + mouseX * -10, node.y + mouseY * -10, 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.strokeStyle = i % 4 === 0 ? `rgba(232, 121, 249, ${alphaPulse * 0.4})` : `rgba(199, 125, 255, ${alphaPulse * 0.4})`;
        ctx.lineWidth = 1;
        ctx.beginPath();
      ...
      ctx.fillStyle = 'rgba(199, 125, 255, 0.008)';
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-[#0A0810]">
